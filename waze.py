...
def rutas(ciudades, inicio, fin):
    ...
    print("Las Rutas posibles son",list(itertools.permutations(ciudades,5)))
    print("El total de rutas es:",len(list(itertools.permutations(ciudades,5))))
    for i in opciones:
        if (i[0] == inicio and i[4] == fin):
            validas.append(i)
    print(f"El numero de rutas validas es: {len(validas)}")
    print()
    return validas
